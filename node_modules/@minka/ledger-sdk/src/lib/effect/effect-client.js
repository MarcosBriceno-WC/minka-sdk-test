...
class EffectClient extends base_client_1.BaseClient {
     ...
     * @param id effect identifier (optional)
     * @returns a builder instance
     */
    drop(id) {
        const EffectIdentifier = id || this.id;
        const builder = new drop_record_builder_1.DropRecordBuilder(async (record, reqOptions) => {
            return this.dropRecord(`${EFFECTS_PATH}/${encodeURIComponent(EffectIdentifier)}`, record, reqOptions);
        }, async (options) => {
            const response = await this.read(EffectIdentifier, options);
            return {
                data: response.effect,
                hash: response.hash,
                luid: response.luid,
                meta: response.meta,
            };
